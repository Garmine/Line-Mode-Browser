...
public abstract class HttpClient {
	...
	protected HttpResponseType defaultType = HttpResponseType.NULL;
	...
	protected HashMap<HttpResponseType, Parser> defaultParsers = new HashMap<HttpResponseType, Parser>();
	...
	private HashMap<Long, HttpRequest> requests = new HashMap<Long, HttpRequest>();
	/** Contains unsent but initialized Requests. Key is the Thread's id for concurrency. */
	private HashMap<Long, HttpRequest> stateMachine = new HashMap<Long, HttpRequest>();
	
	//=======================================================
	//Constructor
	/** Constructs a new Network, and initializes default defaultParsers */
	public HttpClient(){
		defaultParsers.put(HttpResponseType.NULL, null);
		defaultParsers.put(HttpResponseType.SOURCE, null);
		defaultParsers.put(HttpResponseType.JSON, new JsonParser());
	}
	 ...
	 */
	public void setDefaultType(HttpResponseType defaultType){
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public void setDefaultParser(HttpResponseType type, Parser parser){
	 ...
	 */
	public void delDefaultParser(HttpResponseType type){
	 ...
	 */
	public HttpResponseType getDefaultType(){
		return defaultType;
	}
	/**
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public Parser getDefaultParser(HttpResponseType type){
	 ...
	 */
	private HttpRequest waitFor(HttpRequest request){
	 ...
	 */
	protected HttpRequest request(String url, byte[] body, HttpMethod method){
		if(stateMachine.containsKey(ctid())){
			throw new IllegalStateException("ERROR: request is already initialized!");
		}
		HttpRequest ret = new HttpRequest(url, body, method, this);
	...
	protected long send(){
		checkRequest();
		HttpRequest request = stateMachine.remove(ctid());
		request.send();
	 ...
	 */
	protected HttpRequest sendWait(){
		checkRequest();
		HttpRequest request = stateMachine.remove(ctid());
		request.send();
		//wait for request
	 ...
	 */
	protected HttpRequest getResponse(long id){
		if(!isReady(id)){
	 ...
	 * @return the preprocessed response for post processing
	 */
	protected HttpRequest waitResponse(long id){
		if(!isReady(id)){
			//raise error if request non-exists!
			//wait if isn't ready
	 ...
	 */
	protected abstract HttpRequest preProcessResponse(HttpRequest in);
}